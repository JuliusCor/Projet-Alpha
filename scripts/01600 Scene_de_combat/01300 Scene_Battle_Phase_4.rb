...
#    $game_temp.battle_main_phase = true #nécessaire ?
  ...
  def update_phase4
    ...
    if @phase4_step>=@actions.size
      @e_remaining_pk.visible = @e_window_Balls.visible = true if $game_temp.trainer_battle
      end_turn_actions
      #"Netoyage" des effets
      clean_effect($actors)
...
#    end
  ...
  def phase4_switch_pokemon(action)
    ...
    @e_remaining_pk.visible = @e_window_Balls.visible = false if $game_temp.trainer_battle
    Graphics.wait(20)
