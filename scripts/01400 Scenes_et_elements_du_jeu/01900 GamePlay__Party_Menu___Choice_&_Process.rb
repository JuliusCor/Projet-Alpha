module GamePlay
  class Party_Menu < Base
    ...
    def show_menu_mode_choice
      ...
      # @type [PFM::Pokemon]
      pokemon = @party[@index]
      choices = PFM::Choice_Helper.new(Window_Choice, true, 999)
      unless pokemon.egg?
      ...
      choices.register_choice(_get(23, 1), on_validate: method(:hide_winText)) # Cancel
      show_winText(_parse(23, 30, ::PFM::Text::PKNICK[0] => pokemon.given_name))
      x, y = get_choice_coordinates(choices)
      choice = choices.display_choice(@viewport, x, y, 200, choices, on_update: method(:update_menu_choice))
    ...
    def get_choice_coordinates(choices)
      ...
      height += 7
      max_height = 288
      but_x = 100#@team_buttons[@index].x + 53
      but_y = 288 - height #@team_buttons[@index].y + 32
    ...
    def show_battle_mode_choice
      ...
      show_winText(_parse(23, 30, ::PFM::Text::PKNICK[0] => pokemon.given_name))
      x, y = get_choice_coordinates(choices)
      choices.display_choice(@viewport, x, y, 200, choices, on_update: method(:update_menu_choice))
    end
